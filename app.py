...
# ==========================================
# 2. THE S&P 500 UNIVERSE BUILDER (Fixed)
# ==========================================
@st.cache_data(ttl=86400) # Cache this list for 24 hours
def get_sp500_universe():
    """Dynamically fetches the current S&P 500 companies and their sectors."""
    url = 'https://en.wikipedia.org/wiki/List_of_S%26P_500_companies'
    
    # Disguise the script as a standard web browser to bypass Wikipedia's bot block
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
    }
    
    # Fetch the page content safely using the requests library
    response = requests.get(url, headers=headers)
    
    # Pass the HTML text directly to pandas
...
# Fetch the universe in the background (This will now run smoothly)
try:
    sp500_tickers, sp500_sectors = get_sp500_universe()
except Exception as e:
    st.error(f"Error loading S&P 500 list: {e}")
    sp500_tickers, sp500_sectors = [], {}
...
# Only render the dashboard if the ticker list loaded successfully
if sp500_tickers:
    # --- ROW 1 ---
    ...
    with r2_col3:
        ...
        df_table = pd.DataFrame({
            "Symbol": ["^VIX", "SPY", "QQQ", "IWM", "XLF", "XLK", "XLE"],
    ...
    with r2_col4:
        ...
        fig.update_layout(
            title="Net New 52-Week Highs (Highs - Lows)", height=300, 
